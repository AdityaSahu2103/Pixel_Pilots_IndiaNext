...
const navItems = [
  ...
  { href: '/threat-scanner', label: 'Threat Scanner', icon: Shield },
  { href: '/email-analyzer', label: 'Email Analyzer', icon: Mail },
  { href: '/url-scanner', label: 'URL Scanner', icon: Link2 },
  { href: '/ai-intel', label: 'AI Intelligence', icon: Brain },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-5 py-6 border-b"
        ...
        <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 animate-glow-pulse"
        ...
        <AnimatePresence>
          {!collapsed && (
            ...
            >
              ...
              <p className="text-[10px] tracking-widest" style={{ color: '#8B5CF6' }}>
                AI SECURITY
              </p>
            </motion.div>
          )}
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <motion.div
                whileHover={{ x: 4, backgroundColor: 'rgba(0, 245, 255, 0.05)' }}
                whileTap={{ scale: 0.98 }}
