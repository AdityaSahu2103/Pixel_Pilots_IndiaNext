...
import { Shield, AlertTriangle, CheckCircle, Copy, MessageSquare } from 'lucide-react';
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
        ...
        <GlassCard className="mb-6">
          ...
          <div className="mb-3">
            ...
            <input
              type="text"
              placeholder="e.g., Chat message, Customer support, Social media post"
              value={context}
          ...
          <div className="mb-4">
            ...
            <textarea
              placeholder="Paste the text or message content to analyze for threats..."
              value={text}
          ...
          <div className="flex items-center justify-between">
            ...
            <ScanButton onClick={handleScan} loading={loading} label="Scan Text" />
          </div>

          {loading && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4">
              <AnimatedProgress progress={scanProgress} label="Scanning Text" color="#22C55E" />
            </motion.div>
          )}
        </GlassCard>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              <GlassCard>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold" style={{ fontFamily: 'Orbitron', color: '#E5E7EB' }}>Scan Results</h3>
              ...
              {result.detections?.length > 0 && (
                <GlassCard>
                  <h3 className="text-sm font-semibold mb-4" style={{ fontFamily: 'Orbitron', color: '#E5E7EB' }}>Detections</h3>
                  <div className="space-y-3">
                    {result.detections.map((det, i) => (
                      <div key={i} className="p-3 rounded-xl flex items-center justify-between"
                        style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.04)' }}>
                        <div className="flex items-center gap-3">
                          {det.detected ? <AlertTriangle className="w-4 h-4" style={{ color: '#FF3B3B' }} /> : <CheckCircle className="w-4 h-4" style={{ color: '#22C55E' }} />}
                          <div>
                            <p className="text-xs font-semibold capitalize" style={{ color: '#E5E7EB' }}>{det.threat_type.replace('_', ' ')}</p>
                            <p className="text-[10px]" style={{ color: '#6B7280' }}>{Math.round(det.confidence * 100)}% confidence</p>
                          </div>
                        </div>
              ...
              {result.explanation && (
                <GlassCard glow="purple">
                  <h3 className="text-sm font-semibold mb-3" style={{ fontFamily: 'Orbitron', color: '#E5E7EB' }}>🤖 AI Analysis</h3>
                  <p className="text-sm leading-relaxed" style={{ color: '#9CA3AF' }}>{result.explanation.summary}</p>
              ...
              <div className="flex items-center justify-end gap-2">
                <span className="text-[10px] font-mono" style={{ color: '#4B5563' }}>Scan ID: {result.scan_id}</span>
